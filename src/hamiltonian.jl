...
#
# 自旋算符代数（SpinOpTerm / SpinOpSum）的**类型定义**已上移至
# `QuantumCircuits.Hamiltonian`（随 `using QuantumCircuits` 可用）；
# 其状态作用 / 期望值方法在 `ext/quantumcircuits/spinop.jl`
# （VQCQuantumCircuitsExt 扩展，`using QuantumCircuits` 时自动加载）。
# Pauli 代数（PauliTerm / PauliSum）的期望值见同一扩展。

"""
    expectation(m::AbstractMatrix, s::Union{StateVector,DensityMatrix}) -> Complex

一般（可显式构造的）算符期望值：`⟨ψ|m|ψ⟩` 或 `tr(ρ m)`。
大矩阵直接走 BLAS；小局域算符建议用局域核（见 Pauli / SpinOp 接口层方法）。
"""
